...
import { Sparkles, Save, Layout, Type, Image as ImageIcon, Link as LinkIcon, Star } from "lucide-react";

interface ManageHeroProps {
  hero: any;
}

export default function ManageHero({ hero }: ManageHeroProps) {
  ...
  return (
    <AdminLayout>
      ...
      <div className="mb-10 flex justify-between items-end">
        <div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tight mb-2 uppercase">Hero Section</h2>
          <p className="text-slate-500 font-medium">Update the first thing your visitors see.</p>
        </div>
        <button 
      ...
      <div className="grid lg:grid-cols-2 gap-10">
          ...
          <div className="grid gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                <Star size={14} className="text-amber-500" /> Badge Text
              </label>
              <input 
                type="text" 
                value={data.badge}
              ...
              />
              {errors.badge && <p className="text-red-500 text-[10px] font-bold uppercase">{errors.badge}</p>}
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                  <Type size={14} className="text-cyan-500" /> Title Start
                </label>
                <input 
                  type="text" 
                  value={data.title}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                  <Type size={14} className="text-amber-500" /> Highlighted
                </label>
                <input 
                  type="text" 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                  <Type size={14} className="text-cyan-500" /> Title End
                </label>
                <input 
                  type="text" 
                  value={data.titleEnd}
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                <Layout size={14} className="text-emerald-500" /> Subtitle Description
              </label>
              <textarea 
                rows={4}
                value={data.subtitle}
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                  <LinkIcon size={14} className="text-cyan-500" /> Primary CTA
                </label>
                <input 
                  type="text" 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                  <LinkIcon size={14} className="text-slate-400" /> Secondary CTA
                </label>
                <input 
                  type="text" 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                <ImageIcon size={14} className="text-cyan-500" /> Background Image URL
              </label>
              <div className="flex gap-4">
                ...
                />
                <div className="w-16 h-14 bg-slate-100 rounded-2xl overflow-hidden border border-slate-200">
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 rounded-[2.5rem] p-4 shadow-2xl relative overflow-hidden group border-8 border-slate-800">
            <div className="bg-white/5 rounded-[2rem] overflow-hidden relative min-h-[500px] flex items-center p-10">
              ...
              <div className="relative z-10 space-y-6 max-w-lg">
                <div className="inline-flex items-center gap-2 bg-amber-500/20 border border-amber-500/30 text-amber-400 text-[8px] font-black px-3 py-1.5 rounded-full tracking-[0.2em] uppercase">
                    <Sparkles size={10} className="animate-pulse" /> {data.badge}
                </div>
                <h1 className="text-3xl md:text-4xl font-black text-white leading-tight tracking-tight">
                    {data.title} <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400">{data.titleHighlight}</span> {data.titleEnd}
                </h1>
                <p className="text-slate-400 text-sm leading-relaxed font-medium line-clamp-3">
                    {data.subtitle}
                </p>
                <div className="flex gap-4">
                    <div className="px-6 py-3 bg-gradient-to-r from-emerald-600 to-cyan-700 rounded-xl text-[10px] font-black uppercase text-white shadow-xl">
                        {data.ctaPrimary}
                    </div>
                    <div className="px-6 py-3 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase text-white">
                        {data.ctaSecondary}
                    </div>
                </div>
              </div>
          ...
          <div className="bg-amber-50 border border-amber-100 p-8 rounded-[2.5rem]">
            ...
            <p className="text-amber-700 text-sm leading-relaxed font-medium">
                Keep your title short and impactful. Use the <strong>Highlighted</strong> field for the most important keyword to make it stand out with a beautiful gradient.
            </p>
          </div>
        </div>
      </div>
