...
export default async function Home() {
  ...
  const { hero, about, courses, testimonials, settings, homepage } = data;
  ...
  const renderSection = (section: any) => {
    ...
    switch (section.id) {
              ...
              <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
                  ...
                  <AnimatedSection delay={0.3}>
                    <div className="flex flex-wrap justify-center lg:justify-start gap-3 mb-8">
                      <Link href="/admissions" className="btn-gold px-8 py-3.5 text-sm flex items-center gap-2">
                        {hero.ctaPrimary || "Start Your Journey"} <ArrowRight size={16} />
                      </Link>
                ...
                <AnimatedSection delay={0.2} direction="right">
                  <div className="relative mt-12 lg:mt-0">
                    <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-6 md:p-8 shadow-2xl max-w-sm mx-auto lg:max-w-none text-center">
                      <Image
      ...
      case "about":
        return (
          <section key="about" className="py-24 bg-white">
              ...
              <div className="grid lg:grid-cols-2 gap-16 items-center">
                ...
                <AnimatedSection direction="right">
                  ...
                  <h2 className="heading-display mb-6">
                    ...
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-700 to-teal-600">{about.titleHighlight || "IT Revolution"}</span>
                  </h2>
      ...
      case "courses":
        return (
          <section key="courses" className="py-24 bg-slate-50">
            <div className="w-full max-w-7xl mx-auto px-4 lg:px-6">
              <AnimatedSection>
      ...
      case "why-us":
        return (
          <section key="why-us" className="py-24 mesh-gradient text-white relative overflow-hidden">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  ...
                  <AnimatedSection key={i} delay={i * 0.1}>
                    <div className={`bg-gradient-to-b ${item.accent} rounded-xl p-6 border backdrop-blur-sm hover:scale-105 transition-all duration-300 h-full`}>
  ...
  // Build dynamic marquee text if enabled
  let finalMarqueeText = settings.marqueeText;
  if (settings.showAdmissionsInMarquee) {
    const openCourses = courses.filter((c: any) => c.admissionsOpen !== false);
    if (openCourses.length > 0) {
      const admissionItems = openCourses.map((c: any) => 
        `Admissions Open for ${c.title} <a href="/admissions?course=${encodeURIComponent(c.title)}" class="btn-marquee">Apply Now</a>`
      );
      const admissionMsg = admissionItems.join(" • ");
      finalMarqueeText = admissionMsg + (settings.marqueeText ? " • " + settings.marqueeText : "");
    }
  }

  return (
    <div className="overflow-hidden">
      {settings.marqueeShow && finalMarqueeText && (
        <Marquee text={finalMarqueeText} />
      )}
